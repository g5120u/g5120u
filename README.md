...
| Step | 我負責的事 | 可被追問的證據 |
|---|---|---|
| 需求拆解 | 把需求拆成角色、狀態、畫面與限制 | 能說明為什麼這樣切流程 |
| 流程設計 | 先定義資料流與操作路徑，再進入實作 | 能指出狀態從哪裡來、往哪裡去 |
| 程式差異閱讀 | 使用 AI 加速時，我仍會讀 diff、追 call path、檢查副作用 | 能解釋改了哪裡、為什麼改 |
| 行為驗證 | 用實機畫面、log、截圖、回歸檢查確認結果 | 能拿出驗證方式與問題排查紀錄 |
| 交付掌控 | 只交付我理解過、驗證過的變更 | 能現場走讀選定片段 |
